...
TRACK_OVERVIEWS = {
    ...
    "track16-the-penalty-system": (
        "You can earn enough, spend carefully, pay every bill you owe, and still lose over a thousand dollars a year — because the rent is due on the 1st and your wages arrive on the 10th. The charges you pick up for that gap don't distinguish between someone who has no money and someone whose money is nine days away. They cost the same either way. This track is about that difference, and about two fixes that end it without changing a single amount you earn or spend.",
        "It covers what one missed day really costs once both your bank and the company you owe have charged you, why having a payment refused is usually worse than being allowed to go overdrawn, and how the identical mistake costs nothing at one bank and $108 at another. It works out the point where charges stop merely slowing down a debt and start making it grow instead. Then it applies all of that to the situations where it decides how things turn out: people paid once a month on a date set by their birthday, which can't be changed, and people paid every two weeks whose payday keeps drifting against bills that don't move. It deliberately leaves out the comfortable version of this subject, and says so — the same problem that costs a well-off household a few dollars of interest can take everything a tight one has spare.",
    ),
...
CHAPTER_META: dict[str, dict[str, tuple[str, int]]] = {
    ...
    "track16-the-penalty-system": {
        "16.1": ("Bringing in $3,200 against $3,000 of bills leaves you $200 to the good every month — and you can still pay hundreds a year in charges, because the rent goes out on the 1st and your wages arrive on the 10th. That isn't a shortage of money, it's the wrong order, and it needs a different fix: you can't budget your way out of a date. A budget adds up a whole month; a bank charges you on a single day.", 3),
        "16.2": ("Going overdrawn costs $26.77 on average and having a payment refused costs $16.82 — which makes being refused look like the better outcome, and it usually isn't, because the bill still hasn't been paid and the company you owe charges you too. The credit card is the one to remember: your bank and the card company both charge you for the same missed morning, and because banks will do it up to three times a day, an ordinary run of payments can cost over $100 before lunch.", 2),
        "16.3": ("Keep the household the same and change only the bank, and one bad morning costs nothing, or $20, or $108. The banks that stopped charging are almost all online-only; the ones still charging $35 are the ones with branches — so the change mostly helped customers who rarely got charged in the first place. Fewer than 9% of accounts produce close to 80% of what banks collect this way, and half that group has under $350 in the bank.", 5),
        "16.4": ("Every balance has a payment that gets you nowhere — what you owe, times the rate, divided by twelve. On $5,000 at 24.99% it's $104, and getting near it isn't a gentle slope but a wall: cutting your payment by 45% makes it take four times as long. Charges of $96 a month are enough to push someone with $200 spare over that line, which is the difference between clear in three years and never clear at all.", 6),
        "16.5": ("Most people treat a due date like the amount — as a fact about the world. For credit cards, utilities, phone and insurance it's just a setting you can change for free, in minutes, for good. Move the card first, to three to five days after payday, then the utilities, then look again. And when that still doesn't close the gap, ask to split a bill across two paydays, or move somewhere that being short costs less — rather than trying to save your way out of it.", 2),
        "16.6": ("Social Security pays on a date set by your birthday — and against rent due on the 1st, that accident decides whether you hold the money for two days or three weeks. It's the hardest situation in the track: one payment a month with nothing to fall back on, and a date that can't be changed by asking, negotiating or explaining hardship. Every fix has to happen on the bills — and the cheapest fix in personal finance is also the one most likely to be defeated by a phone menu.", 4),
        "16.7": ("Being paid every two weeks means payday drifts about two days a month against bills that don't move, which is why an arrangement that worked for four months suddenly fails and nothing appears to have changed. The two months a year with three paychecks are the realistic answer: $1,800 of found money that nobody on a tight budget could ever save out of ordinary months, and enough to get in front of the bills for good — if you work out which months those are in advance instead of spending it without noticing.", 1),
        "16.8": ("Find the lowest your balance gets, look up what your bank charges, move the card, move the utilities, look again — and stop as soon as you're staying above zero, which for most people is three phone calls in. Change banks before trying to save your way out, and get a month ahead last, using a third paycheck rather than a month that hasn't got it spare. And if you haven't been charged all year, skip the lot: this was written for the household where three phone calls are worth more than a pay rise.", 5),
    },
    "volume2-track1-your-built-in-wiring": {
